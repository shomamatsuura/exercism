...
namespace troy {
void give_new_artifact(human& person, const std::string& new_name){
    person.possession = std::make_unique<artifact>(artifact(new_name));
}
...
int power_intensity(human& person){
    ...
    // return person.own_power.use_count() + person.influenced_by.use_count();
